...
const L_COLOR = { High: '#f87171', Med: '#fbbf24', Low: '#34d399' };
const L_RGB   = { High: [248,113,113], Med: [251,191,36], Low: [52,211,153] };
...
// ── Boundary renderer — pixel-level with glowing edges ────────────────────────
function renderBoundary(canvas, tree, W, H) {
  ...
  // Classify every pixel (step 2 for perf)
  const STEP = 2;
  const lmap = new Uint8Array(W * H); // 0=High,1=Med,2=Low
  ...
  const rgbs = [L_RGB.High, L_RGB.Med, L_RGB.Low];
  ...
  for (let py=0; py<H; py++) {
    for (let px=0; px<W; px++) {
      ...
      // Check 8-neighbourhood for boundary
      let diff = 0;
      for (let dy=-1; dy<=1; dy++) for (let dx=-1; dx<=1; dx++) {
        if (!dy&&!dx) continue;
        const ny=py+dy, nx=px+dx;
        if (ny>=0&&ny<H&&nx>=0&&nx<W&&lmap[ny*W+nx]!==m) diff++;
      }

      const i = (py*W+px)*4;
      if (diff > 0) {
        // glowing boundary line
        const t = Math.min(1, diff/4);
        buf[i]   = Math.min(255, r + 160*t);
        buf[i+1] = Math.min(255, g + 160*t);
        buf[i+2] = Math.min(255, b + 160*t);
        buf[i+3] = 255;
      } else {
        // tinted fill with subtle grain
        const grain = (Math.sin(px*0.41+py*0.23)*0.5+0.5)*10;
        buf[i]   = Math.min(255, r*0.15 + grain);
        buf[i+1] = Math.min(255, g*0.15 + grain);
        buf[i+2] = Math.min(255, b*0.15 + grain);
        buf[i+3] = 210;
      }
    }
  }
  ...
  // Glow pass: draw blurred copy on top
  ctx.save();
  ctx.filter = 'blur(5px)';
...
// ── Tree layout engine — computes node positions so nothing overlaps ──────────
const NW=66, NH=22, YGAP=46, XPAD=8;
    ...
    <g>
      {node.parent_cx!==undefined && (
        <line
          ...
          stroke="#ffffff12" strokeWidth={1.2} strokeDasharray="4 3"
        />
      )}
      <rect
        x={node.cx-NW/2} y={node.y-NH/2} width={NW} height={NH}
        rx={isLeaf ? 11 : 5}
        fill={isLeaf ? col+'1e' : '#0f1022'}
        stroke={isLeaf ? col : '#2a2d4a'}
        strokeWidth={1.3}
      />
      <text
...
function TreeViz({ tree, accentColor }) {
  ...
  return (
    <div style={{overflowX:'auto', WebkitOverflowScrolling:'touch', scrollbarWidth:'thin', scrollbarColor:'#ffffff15 transparent'}}>
      <svg
        viewBox={`0 0 ${svgW} ${svgH}`}
        width={svgW} height={svgH}
        style={{display:'block', minWidth:svgW, maxWidth:'100%'}}
      >
...
function Tooltip({point, predicted, pxX, pxY, maxW}) {
  ...
  return (
    <div style={{
      ...
      <div style={{marginTop:5,color:correct?'#34d399':'#f87171',fontWeight:700,fontSize:'0.58rem'}}>
...
export default function Overfitting() {
  ...
  const status = depth<=2
    ...
    : depth<=4
    ? {label:'Optimal',      color:'#34d399', desc:'Good balance of bias vs variance' }
    : {label:'Overfitting',  color:'#f87171', desc:'Memorising noise, not patterns'   };
  ...
  const deeTree = buildTree(data,0,7);

  return (
    <div style={{
      maxWidth:660, margin:'0 auto', padding:'1.1rem 0.8rem 3rem',
      fontFamily:"'Space Mono',monospace", color:'#e5e7eb',
      background:'#070710', minHeight:'100vh',
    }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&display=swap');
        *,*::before,*::after{box-sizing:border-box}

        .card{background:#0c0c1b;border:1px solid #ffffff0b;border-radius:16px;padding:.95rem 1rem;margin-bottom:.9rem}
        .ey{font-size:.53rem;letter-spacing:.13em;color:#2e2e50;text-transform:uppercase;margin-bottom:.6rem}
        ...
        .plot-wrap{position:relative;width:100%;aspect-ratio:1;border-radius:12px;
        ...
        input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:18px;height:18px;
          border-radius:50%;border:2.5px solid #070710;cursor:pointer}
        ...
        .cmp-grid{display:grid;grid-template-columns:1fr 1fr;gap:.6rem}
        @media(max-width:360px){.cmp-grid{grid-template-columns:1fr}}
      `}</style>

      {/* HEADER */}
      <div style={{marginBottom:'1.1rem',paddingLeft:'0.05rem'}}>
        <div style={{fontSize:'.52rem',letterSpacing:'.17em',color:'#2e2e50',marginBottom:5}}>MACHINE LEARNING · INTERACTIVE</div>
        <h1 style={{margin:0,fontSize:'clamp(.95rem,4vw,1.15rem)',fontWeight:700,color:'#f9fafb',lineHeight:1.2}}>
          Decision Tree · Overfitting
        </h1>
        <p style={{margin:'.28rem 0 0',fontSize:'.58rem',color:'#2e2e50'}}>
      ...
      <div style={{
        ...
        borderRadius:16,border:`1px solid ${status.color}25`,
        background:`${status.color}0a`,marginBottom:'.9rem',transition:'all .35s',
      }}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:'.57rem',color:'#4b5563',marginTop:2}}>{status.desc}</div>
        </div>
        <div style={{textAlign:'right',flexShrink:0}}>
          <div style={{fontSize:'.48rem',color:'#2e2e50',letterSpacing:'.1em'}}>DEPTH</div>
      ...
      {/* ACCURACY */}
      <div className="card">
        <div className="ey" style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'.7rem'}}>
          <span>Accuracy</span>
          <span style={{
            padding:'.13rem .5rem',borderRadius:99,fontSize:'.52rem',letterSpacing:'.07em',
            background:gap>20?'#f8717115':'#ffffff08',
            color:gap>20?'#f87171':'#2e2e50',
            border:`1px solid ${gap>20?'#f8717128':'#ffffff0a'}`,
          }}>Gap {gap}% {gap>20?'⚠':'✓'}</span>
        </div>
        {[
          {label:'Train', value:trainAcc, color:'#818cf8'},
          {label:'Test',  value:testAcc,  color:'#34d399'},
      ...
      <div className="card">
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:'.4rem',alignItems:'center',marginBottom:'.7rem'}}>
          ...
          <button className={`pill ${adding?'on':''}`}
            onClick={()=>setAdding(a=>!a)}
            style={adding?{color:'#34d399',borderColor:'#34d39938',background:'#34d39910'}:{}}>
            {adding?'✦ Adding':'+ Add'}
          </button>
          <button className={`pill ${animate?'on':''}`}
            onClick={()=>setAnimate(a=>!a)}
            style={animate?{color:'#f87171',borderColor:'#f8717138',background:'#f8717110'}:{}}>
            {animate?'⏹':'▶'}
          </button>
        ...
        {adding && (
          <div style={{display:'flex',gap:'.38rem',marginBottom:'.6rem',alignItems:'center',flexWrap:'wrap'}}>
            <span style={{fontSize:'.52rem',color:'#2e2e50'}}>Class:</span>
            {LABELS.map(l=>(
            ...
            <span style={{fontSize:'.5rem',color:'#2e2e50'}}>· tap plot</span>
          </div>
        )}
            ...
            {data.map((d,i)=>{
              ...
              return (
                <g key={i} style={{cursor:'pointer'}}
                  ...
                  {hov&&<circle cx={cx} cy={cy} r="15" fill={col+'20'}/>}
                  {!ok&&<circle cx={cx} cy={cy} r="10" fill="none" stroke="#fff" strokeWidth={1.3}
                    strokeDasharray="3 2.5" opacity={0.45}/>}
                  <circle cx={cx} cy={cy} r={hov?8:5.5}
                    fill={col}
                    stroke={ok?'rgba(255,255,255,0.2)':'#fff'}
                    strokeWidth={ok?1:1.8}
                    filter={hov?`url(#g${d.label})`:undefined}
                  />
                  {!ok&&<>
                    <line x1={`calc(${cx} - 3.5px)`} y1={`calc(${cy} - 3.5px)`}
                          x2={`calc(${cx} + 3.5px)`} y2={`calc(${cy} + 3.5px)`}
        ...
        <div style={{display:'flex',justifyContent:'center',marginTop:6}}>
          <span style={{fontSize:'.5rem',color:'#2e2e50'}}>→ Glucose (normalised)  ↑ BMI</span>
        </div>
          ...
          {LABELS.map(l=>(
            <div key={l} style={{display:'flex',alignItems:'center',gap:5,fontSize:'.56rem',color:'#6b7280'}}>
              <div style={{width:8,height:8,borderRadius:'50%',background:L_COLOR[l],
                           boxShadow:`0 0 6px ${L_COLOR[l]}80`}}/>{l} Risk
            </div>
          ))}
        ...
        <p style={{textAlign:'center',fontSize:'.5rem',color:'#1e1e35',margin:'.35rem 0 0'}}>
      ...
      <div className="card">
        ...
        <input type="range" min={1} max={8} value={depth}
          ...
          style={{
            ...
            background:`linear-gradient(to right,${status.color}65 ${((depth-1)/7)*100}%,#ffffff0d ${((depth-1)/7)*100}%)`,
          }}
        />
        <div style={{display:'flex',justifyContent:'space-between',marginTop:5}}>
          <span style={{fontSize:'.5rem',color:'#fbbf24'}}>← Underfit</span>
          <span style={{fontSize:'.5rem',color:'#34d399'}}>Optimal 3–4</span>
          <span style={{fontSize:'.5rem',color:'#f87171'}}>Overfit →</span>
        </div>
      </div>
      ...
      <div className="card">
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'.6rem'}}>
          ...
          <span style={{fontSize:'.54rem',color:'#2e2e50'}}>{trainAcc}% train · {testAcc}% test</span>
        </div>
        <div style={{
          background:'#070710',borderRadius:10,
        ...
        <p style={{textAlign:'center',fontSize:'.5rem',color:'#1e1e35',margin:'.35rem 0 0'}}>
          ← scroll horizontally to see full tree →
        </p>
      </div>

      {/* COMPARISON */}
      <div className="card">
        <div className="ey">Structure Comparison</div>
        <div className="cmp-grid">
          {[
            {label:'Shallow · depth 2', t:simTree, color:'#fbbf24', tag:'Underfit'},
            {label:'Deep · depth 7',    t:deeTree, color:'#f87171', tag:'Overfit' },
          ].map(({label,t,color,tag})=>(
            <div key={label} style={{borderRadius:10,border:`1px solid ${color}18`,background:`${color}07`,padding:'.6rem'}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:7}}>
                <span style={{fontSize:'.55rem',color}}>{label}</span>
                <span style={{fontSize:'.53rem',color:'#2e2e50'}}>{accuracy(t,data)}%</span>
              </div>
              <div style={{background:'#070710',borderRadius:8,border:`1px solid ${color}12`,
                           padding:'6px 12px 4px',overflowX:'auto',WebkitOverflowScrolling:'touch',
                           scrollbarWidth:'thin',scrollbarColor:'#ffffff12 transparent'}}>
                <TreeViz tree={t} accentColor={color}/>
              </div>
              <div style={{textAlign:'center',marginTop:6,fontSize:'.57rem',color,fontWeight:700}}>{tag}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{borderColor:'#ffffff07'}}>
        <div className="ey">What's happening?</div>
